  ...
  return (
    <DashboardLayout
      ...
      action={
        ...
        >
          ...
          <span className="hidden sm:inline">Analyze Another Job</span>
    ...
    >
      <div className="max-w-5xl space-y-5 sm:space-y-6">
        ...
        <div className="bg-gradient-to-r from-gray-900 via-emerald-950 to-emerald-900 text-white rounded-3xl p-5 sm:p-8 shadow-xl relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-5 sm:gap-6">
            <div className="space-y-2.5 sm:space-y-3 max-w-xl">
              ...
              <div className="inline-block px-3 py-1 rounded-full text-[11px] sm:text-xs font-bold border border-white/10 bg-white/10 text-emerald-200">
            ...
            <div className="bg-slate-950/60 border border-emerald-500/30 backdrop-blur-xl rounded-3xl p-5 sm:p-6 flex flex-col items-center justify-center shrink-0 w-32 h-32 sm:w-36 sm:h-36 mx-auto md:mx-0 shadow-2xl">
              <div className={`text-3xl sm:text-4xl font-black ${getScoreColor(overallScore)}`}>
                {overallScore}%
              </div>
              <span className="text-[9px] sm:text-[10px] text-gray-400 font-bold uppercase tracking-wider mt-1">Match Score</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 sm:gap-4">
          <div className="bg-white border border-gray-100 rounded-2xl p-4 sm:p-5 shadow-sm space-y-2">
            <div className="flex items-center justify-between text-xs font-bold">
              <span className="text-gray-500 uppercase tracking-wider">Skills Overlap</span>
              <span className="text-emerald-600 font-extrabold">{skillsScore}%</span>
            </div>
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-4 sm:p-5 shadow-sm space-y-2">
            <div className="flex items-center justify-between text-xs font-bold">
              <span className="text-gray-500 uppercase tracking-wider">Experience</span>
              <span className="text-blue-600 font-extrabold">{expScore}%</span>
            </div>
            <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
              <div className="h-2.5 bg-blue-500 rounded-full transition-all duration-500" style={{ width: `${expScore}%` }} />
            </div>
            <p className="text-[11px] text-gray-400">{resume.experience_years || 0} yrs vs required</p>
          </div>

          <div className="bg-white border border-gray-100 rounded-2xl p-4 sm:p-5 shadow-sm space-y-2">
            <div className="flex items-center justify-between text-xs font-bold">
              <span className="text-gray-500 uppercase tracking-wider">Semantic Vector</span>
              <span className="text-purple-600 font-extrabold">{semanticScore}%</span>
            </div>
            <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
              <div className="h-2.5 bg-purple-500 rounded-full transition-all duration-500" style={{ width: `${semanticScore}%` }} />
            </div>
            <p className="text-[11px] text-gray-400 truncate">{analysis.semantic_interpretation || 'Deep embedding similarity'}</p>
          </div>
        </div>

        {/* AI Executive Feedback Summary */}
        {analysis.explanation && (
          <div className="bg-gradient-to-r from-emerald-50/60 to-teal-50/40 border border-emerald-200/80 rounded-3xl p-5 sm:p-6 shadow-sm space-y-2">
          ...
          <div className="bg-white border border-gray-100 rounded-3xl p-5 sm:p-6 shadow-sm space-y-4">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  <h2 className="text-sm sm:text-base font-bold text-gray-900 truncate">Matched Skills</h2>
                  <p className="text-[11px] sm:text-xs text-gray-400 truncate">Verified strengths present in resume</p>
                </div>
              </div>
              <span className="text-xs font-extrabold bg-emerald-50 text-emerald-700 px-2.5 sm:px-3 py-1 rounded-full border border-emerald-200 shrink-0">
            ...
            <div className="flex flex-wrap gap-1.5 sm:gap-2 pt-1">
              {(!analysis.matched_skills || analysis.matched_skills.length === 0) && (
                <div className="text-xs text-gray-400 py-3">No matching skills detected in resume.</div>
              )}
              {analysis.matched_skills?.map((s) => (
                <span
                  key={s}
                  className="text-xs bg-emerald-50 text-emerald-700 font-semibold px-2.5 sm:px-3 py-1.5 rounded-xl border border-emerald-200/60 capitalize flex items-center gap-1.5 max-w-full"
                >
          ...
          <div className="bg-white border border-gray-100 rounded-3xl p-5 sm:p-6 shadow-sm space-y-4">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  <h2 className="text-sm sm:text-base font-bold text-gray-900 truncate">Skill Gaps Identified</h2>
                  <p className="text-[11px] sm:text-xs text-gray-400 truncate">Missing requirements for full match</p>
                </div>
              </div>
              <span className="text-xs font-extrabold bg-rose-50 text-rose-700 px-2.5 sm:px-3 py-1 rounded-full border border-rose-200 shrink-0">
            ...
            <div className="flex flex-wrap gap-1.5 sm:gap-2 pt-1">
              {(!analysis.missing_skills || analysis.missing_skills.length === 0) && (
                <div className="text-xs text-emerald-600 font-bold py-3">Awesome! Zero skill gaps found for this role.</div>
              )}
              {analysis.missing_skills?.map((s) => (
                <button
                  key={s}
                  onClick={() => setSelectedSkill(s)}
                  className="text-xs bg-rose-50 hover:bg-rose-100 text-rose-700 font-semibold px-2.5 sm:px-3 py-1.5 rounded-xl border border-rose-200/60 capitalize flex items-center gap-1.5 transition max-w-full"
                  title="Click to view learning syllabus"
                >
                  <AlertCircle className="w-3.5 h-3.5 text-rose-500 shrink-0" />
                  <span className="truncate">{s}</span>
                  <span className="text-[10px] bg-white text-rose-600 font-bold px-1.5 py-0.5 rounded border border-rose-200 shrink-0">
                    Fix →
                  </span>
                </button>
              ))}
        ...
        {roadmap && roadmap.length > 0 && (
          <div className="bg-white border border-gray-100 rounded-3xl p-5 sm:p-6 shadow-sm space-y-4 sm:space-y-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div>
                <h2 className="text-sm sm:text-base font-bold text-gray-900">Tailored Skill Growth Roadmap</h2>
                <p className="text-xs text-gray-400">Click any skill module to access structured syllabi and practice questions</p>
              </div>
              <span className="text-xs bg-emerald-50 text-emerald-700 font-bold px-3 py-1 rounded-full border border-emerald-200 w-fit">
                AI Roadmap Ready
              </span>
            </div>
                ...
                >
                  <div className="space-y-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-bold text-gray-900 capitalize group-hover:text-emerald-700 transition truncate">
                        {item.skill}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      className="flex-1 sm:flex-none text-xs bg-white hover:bg-emerald-600 hover:text-white text-gray-700 font-semibold px-3 py-2 rounded-xl border border-gray-200/80 shadow-xs transition flex items-center justify-center gap-1"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 sm:flex-none text-xs bg-white hover:bg-purple-600 hover:text-white text-gray-700 font-semibold px-3 py-2 rounded-xl border border-gray-200/80 shadow-xs transition flex items-center justify-center gap-1"
                    >
        ...
        <div className="bg-white border border-gray-100 rounded-3xl p-5 sm:p-6 shadow-sm space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 text-xs">
            <div className="bg-gray-50 p-3 rounded-2xl border border-gray-100 min-w-0">
              <span className="text-gray-400 block mb-0.5">Candidate</span>
            ...
            <div className="bg-gray-50 p-3 rounded-2xl border border-gray-100 min-w-0">
              ...
              <span className="font-bold text-gray-800 truncate block">{resume.experience_years || 0} Years</span>
            </div>
            <div className="bg-gray-50 p-3 rounded-2xl border border-gray-100 min-w-0">
              <span className="text-gray-400 block mb-0.5">Resume Skills</span>
              <span className="font-bold text-gray-800 truncate block">{resume.skills_found?.length || 0} Found</span>
            </div>
            <div className="bg-gray-50 p-3 rounded-2xl border border-gray-100 min-w-0">
              <span className="text-gray-400 block mb-0.5">Requirements</span>
              <span className="font-bold text-gray-800 truncate block">{job.required_skills?.length || 0} Required</span>
            </div>
          </div>
        </div>
        ...
        <div className="text-center pt-2 sm:pt-4">
          <button
            onClick={() => navigate('/analyze')}
            className="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs sm:text-sm px-8 py-3.5 rounded-2xl shadow-lg shadow-emerald-600/20 transition transform hover:scale-105 inline-flex items-center justify-center gap-2"
          >
      ...
      {selectedSkill && (
        ...
        >
          <div
            className="bg-white border border-gray-100 rounded-3xl max-w-sm w-full p-5 sm:p-6 shadow-2xl space-y-4"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between border-b border-gray-100 pb-3">
              <div className="font-bold text-gray-900 text-base capitalize truncate">{selectedSkill}</div>
              <button onClick={() => setSelectedSkill(null)} className="text-gray-400 hover:text-gray-600 font-bold text-xl px-2">
                ×
              </button>
            </div>
